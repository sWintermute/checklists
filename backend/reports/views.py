...
class ExcelView(APIView):
    def get(self, request):
        ...
        survey = request.data.get('lists')
        questions = Question.objects.filter(survey=survey)
        responses = Response.objects\
            .prefetch_related('answers', 'answers__question', 'user', 'survey')\
            .filter(survey=survey, created__date__gt=date_from, created__date__lt=date_to)#\
            #.only('id', 'created', 'answers', 'answers__question__text', 'user')
